...
## Contexto do projeto
- Arquitetura em camadas (domínio puro → aplicação → infraestrutura → apresentação). Veja `DOCUMENTAÇÃO/GUIA-DO-CODIGO.md` e `DOCUMENTAÇÃO/ARQUITETURA.md`.
...
5. **Desempenho**: quando relevante, rode `scripts/benchmark.py` e compare com as metas de `DOCUMENTAÇÃO/PLANO-COMERCIALIZACAO.md` (seção Desempenho).
