...
from app.infrastructure.importers.pdfium_importer import PdfiumImporter, classify_kind
...
def test_multipagina_gera_multiplas_artworks(tmp_path):
    ...
    _vec_page(doc)
    _vec_page(doc)
    arts = PdfiumImporter().import_artworks(_save(doc, tmp_path))
    assert len(arts) == 3
...
def test_dimensao_mediabox(tmp_path):
    ...
    art = PdfiumImporter().import_artworks(_save(doc, tmp_path))[0]
...
def test_caixa_de_midia_usa_mediabox(tmp_path):
    ...
    art = PdfiumImporter().import_artworks(_save(doc, tmp_path), box="media")[0]
...
def test_caixa_de_apara_usa_trimbox(tmp_path):
    ...
    art = PdfiumImporter().import_artworks(_save(doc, tmp_path), box="trim")[0]
    assert art.size.width == pytest.approx(300 * PT2MM, abs=0.1)
...
def test_arquivo_invalido_levanta_erro(tmp_path):
    ...
    with pytest.raises(PdfImportError):
        PdfiumImporter().import_artworks(str(bad))


def test_arquivo_inexistente_levanta_erro(tmp_path):
    with pytest.raises(PdfImportError):
        PdfiumImporter().import_artworks(str(tmp_path / "nao_existe.pdf"))
...
def test_arquivo_imagem_nao_e_pdf(tmp_path):
    ...
    pix.save(str(img))
    with pytest.raises(PdfImportError):
        PdfiumImporter().import_artworks(str(img))
